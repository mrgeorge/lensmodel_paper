...
\begin{equation}
\rho_{\star}(r) = \frac{M_{\star}}{2\pi} \frac{a}{r(r+r_H)^3}
\label{eq:hernquist}
\end{equation}
parametrized by the total stellar mass $M_{\star}$ and scale radius
$r_H$. If a constant stellar mass-to-light ratio is assumed, the scale
radius can be measured from the surface brightness profile of the galaxy
and deprojected to estimate $r_H$. Stellar masses are typically
...
function (IMF). We will treat $M_{\rm star}$ as a free parameter to
...
eventually collapsion into a final distribution which we take to be
...
that there is no contraction at or beyond the virial radius. This boundary condition
leads to the result that contraction models producing an excess
mass density near $r \lesssim r_H$ have a mass \textit{deficit} on
intermediate scales between $\sim r_H$ and the virial radius, and vice
...
by $20\%$ from the corresponding NFW model at scales of tens of
kiloparsecs, and the shear and magnification observables have
